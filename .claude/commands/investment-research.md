...
> - A股：东方财富（主）+ 巨潮资讯（副）
> - 한국주식(KRX): DART 공시 MCP (원문·주, `opendart-find_company`→`mcp__dart__get_financial_summary`) + Yahoo Finance `.KS/.KQ` (시장데이터·보조). 주의: 회계연도 절대값은 DART 기준, Yahoo의 Revenue(TTM)는 회계연도와 다름
