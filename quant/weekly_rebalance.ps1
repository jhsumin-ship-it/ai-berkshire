...
# 미국 성장주 모멘텀 리밸런싱(신호) — yfinance 자체수집. 실행은 수동(미국 브로커).
"--- 미국 모멘텀 리밸런싱 ---" | Tee-Object -FilePath $log -Append
python quant/run.py us-rebalance --notify 2>&1 | Tee-Object -FilePath $log -Append
# 최신 리포트 자동 열기(한국 리밸런싱 + 모의투자 + 미국 리밸런싱)
